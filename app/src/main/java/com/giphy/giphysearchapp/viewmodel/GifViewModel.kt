...
class GifViewModel @Inject constructor(
    ...
    // --- Cache for search → details navigation ---
    private val _cachedGifs = MutableStateFlow<Map<String, GifItem>>(emptyMap())
    ...
    fun refreshGifs(query: String) {
        ...
        viewModelScope.launch { fetchGifs(key, state, refresh = true) }
    }
        ...
        viewModelScope.launch {
            ...
            try {
                val flow = when (key) {
                    ...
                    .map { pagingData ->
                        pagingData.map { gif ->
                            // Cache each GIF for details navigation
                            _cachedGifs.value = (_cachedGifs.value + (gif.id to gif)) as Map<String, GifItem>
                            gif
                        }
                    }
    ...
    // --- Safe GIF lookup for Details navigation ---
    fun getGifById(id: String?): GifItem? {
        if (id == null) return null
        _favorites.value.find { it.id == id }?.let { return it }
        _cachedGifs.value[id]?.let { return it }
        return null
    }
